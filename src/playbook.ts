...
- The ticket date is not the incident date: users often open a ticket days after being blocked. Pass incident_time only when the ticket says when it happened ('YYYY/MM/DD HH:MM', or 'YYYY/MM/DD' for a whole day). Otherwise omit it and 'period': diagnose tools then search back window by window (last 24h, 1-2, 2-3, 3-5, 5-7, 7-10, 10-14 days ago by default; lookback_days up to 30 if the block may be older) and stop where the evidence is. Do not conclude 'no block' from the last 24h only. Avoid a single 30-day search: it times out. If a tool says it stopped before the client timeout, call it again with the given lookback_start_days. Pass device_group as soon as it is known: without it, config is read from every device group, which is slow. After a timeout, do not fire several tools in parallel.
