...
describe("lookbackWindows", () => {
  it("covers the last 14 days by default, newest first, without gaps", () => {
    const w = lookbackWindows(NOW);
    expect(w.map((x) => x.label)).toEqual(["last 24h", "1-2 days ago", "2-3 days ago", "3-5 days ago", "5-7 days ago", "7-10 days ago", "10-14 days ago"]);
    expect(w[0].end_time).toBe("2026/09/25 12:00:00");
    expect(w[w.length - 1].start_time).toBe("2026/09/11 12:00:00");
    for (let i = 1; i < w.length; i++) expect(w[i].end_time).toBe(w[i - 1].start_time);
  });

  it("stops at the requested number of days and caps at 30", () => {
    ...
    const max = lookbackWindows(NOW, 60);
    expect(max[max.length - 1].label).toBe("21-30 days ago");
  });
});
